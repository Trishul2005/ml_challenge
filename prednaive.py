"""
Naive Bayes baseline for the ML challenge.
...
def row_tokens(row):
    ...
    tokens.append("price_" + price_bucket(parse_price(row.get(PRICE_COL, ""))))
...
def train_naive_bayes(X, y):
    ...
    for label in range(num_classes):
        mask = (y == label)
        class_counts[label] = numpy.sum(mask)
        if class_counts[label] > 0:
            token_counts[label] = X[mask].sum(axis=0)

    priors = (class_counts + SMOOTHING) / (numpy.sum(class_counts) + SMOOTHING * num_classes)
    likelihoods = (token_counts + SMOOTHING) / (
...
def fit_model():
    ...
    return fit_model_from_rows(train_rows, labels)
...
def predict_all(filename):
    ...
    for test_example in data:
        predictions.append(predict(test_example))
    return predictions
